...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
    ...
    "SELECT s.usuarioSeguido.id FROM Seguidor s WHERE s.usuarioSeguidor.id = :usuarioId) " +
    "AND u.id NOT IN (" +
    "SELECT DISTINCT CASE WHEN m.emisor.id = :usuarioId THEN m.receptor.id ELSE m.emisor.id END " +
